...
def second_most_popular_sentiment(df):
    ...
    #         max = length
    return (
        df["Sentiment"].value_counts().index[1]
    )  # ENSURE THAT THE SECOND MAXIMUM IS THE RETURNED VALUE!!!


# Return the date (string as it appears in the data) with the greatest number of extremely positive tweets.
def date_most_popular_tweets(df):
    possible_dates = set(
        df["TweetAt"].loc[(df["Sentiment"] == "Negative")]
    ...
    for date in possible_dates:
        length = len(
            df["Sentiment"].loc[
                (df["Sentiment"] == "Negative") & (df["TweetAt"] == date)
            ]
        )
...
def remove_multiple_white_spaces(text):  # Remove this one?
    ...
    )  # Fix!, it is still returning the spaces
    return clean_text
...
def remove_non_alphabetic_chars(df):
    df["OriginalTweet"] = df["OriginalTweet"].apply(
        lambda x: remove_punctuation_space(x)
    )
    return df
...
def remove_multiple_consecutive_whitespaces(df):
    df["OriginalTweet"] = df["OriginalTweet"].apply(
        lambda x: remove_multiple_white_spaces(x)
    )
    return df
...
def mnb_predict(df):
    ...
    coun_vect = CountVectorizer(ngram_range=(4, 4))
    x = coun_vect.fit_transform(df["OriginalTweet"])
