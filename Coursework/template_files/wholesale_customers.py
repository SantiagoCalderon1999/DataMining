...
def summary_statistics(df):
	...
	for column in df.columns: # Is there a better way than just using the for?
		data_statistics.append([round(df[column].mean()), round(df[column].std()), df[column].min(), df[column].max()]) # Ensure round works fine
	return pd.DataFrame(data_statistics, columns=['mean', 'std', 'min', 'max']).set_index(df.columns)
...
def standardize(df):
	...
	for column in df.columns:
		standardized_data[column] = (standardized_data[column] - df[column].mean()) / df[column].std()
...
def agglomerative(df, k):
	agg = cluster.AgglomerativeClustering(n_clusters = k, linkage='average') # Add euclidean distance
	agg.fit(df)
    ...
    for data_type in data_types:
        ...
        for k in ks:
            ...
            for i in range(0, number_iterations):
                ...
                SC_cummulative.append(metrics.silhouette_score(df_used, km.labels_, metric = 'euclidean'))
            SC = max(SC_cummulative)
            results.append(['Kmeans', data_type, k, SC])
        for k in ks:
            #km = cluster.AgglomerativeClustering(n_clusters = k, linkage='average', metric='euclidean')
            km = cluster.AgglomerativeClustering(n_clusters = k, linkage='average') # See if Euclidean metric can now be used
...
def scatter_plots(df):
	...
	for i in range(0, len(df.columns)):
		...
		for j in range(i+1, len(df.columns)):
				...
				_plot_cluster(km.labels_, colors, df.loc[:, (df.columns == label_1) | (df.columns == label_2)], k, label_1, label_2)
